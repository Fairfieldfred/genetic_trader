...
class PythonBridge {
  ...
  Future<void> startEvolution({
    ...
    try {
      ...
      // Listen to stderr — only log, don't treat as fatal error.
      // Python writes warnings, deprecation notices, and progress
      // to stderr; these should not abort the evolution run.
      _process!.stderr
          .transform(utf8.decoder)
          .transform(const LineSplitter())
          .listen(
      ...
      // Wait for process to finish — single source of truth for
      // completion/error, avoiding duplicate onComplete calls.
      ...
      if (exitCode == 0) {
        ...
        // Collect recent stderr lines for the error message
        final stderrLines = _outputLines
        ...
        final lastLines = stderrLines.length > 5
            ...
            : stderrLines;
        final msg = lastLines.isNotEmpty
            ? lastLines.join('\n')
